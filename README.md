...
| 区域 / 按钮 | 作用 |
|-------------|------|
| **正在处理** | 类似 CPU 当前进程，同一时间至多一条；无则为「空闲」 |
| **排队中** | 已保存、等待调度的活动任务，按入队时间排序；点选即升为正在处理 |
| **已完成** | 已标记完成的任务（默认保留约 14 天） |
| **Switch** | 新建任务并设为正在处理，原正在处理任务进入排队 |
...
预览页会**始终显示**上述区块；无内容时显示灰色占位提示。富文本区域支持粘贴或拖入图片；**双击图片**可放大预览，预览中 **Ctrl+滚轮** 缩放、**左键拖动** 平移；编辑时 URL 可自动识别为可点击链接。
